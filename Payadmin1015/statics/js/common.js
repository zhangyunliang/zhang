;(function () {
  ...
  //自己加的
    var ajaxForm_my = $('form.J_ajaxForm_myself');
    if (ajaxForm_my.length) {
        Wind.use('ajaxForm', 'artDialog', function () {
            if ($.browser.msie) {
                //ie8及以下，表单中只有一个可见的input:text时，会整个页面会跳转提交
                ajaxForm_my.on('submit', function (e) {
                    //表单中只有一个可见的input:text时，enter提交无效
                    e.preventDefault();
                });
            }

            $('button.J_ajax_submit_myself').on('click', function (e) {
                e.preventDefault();
                /*var btn = $(this).find('button.J_ajax_submit_btn'),
                 form = $(this);*/
                var btn = $(this),
                    form = btn.parents('form.J_ajaxForm_myself');
    ...
    //自己加的
    if ($('a.J_ajax_my').length) {
        Wind.use('artDialog', function () {
            $('.J_ajax_my').on('click', function (e) {
                e.preventDefault();
                var $_this = this,
                    $this = $($_this),
                    href = $this.prop('href'),
...
function open_iframe_dialog(url,title,options,width,height){
    var a = arguments[3] ? arguments[3] :"95%";
    var b = arguments[4] ? arguments[4] :"90%";
