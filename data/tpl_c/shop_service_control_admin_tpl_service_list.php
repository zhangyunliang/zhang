<?php keke_tpl_class::checkrefresh('shop/service/control/admin/tpl/service_list', '1465984254' );?><!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
...
<td colspan="5">
 ...
 <select name="w[ord]">
                     		 	 <option <?php if($w['ord']=='service_id desc' or !isset($w['ord'])) { ?>selected="selected" <?php } ?>  value="service_id desc">ID<?php echo $_lang['desc'];?></option>
                     		 	 <option <?php if($w['ord']=='service_id asc') { ?> selected="selected" <?php } ?>  value="service_id asc">ID<?php echo $_lang['asc'];?></option>
                     		 </select>
...
<input type="hidden" name="page" value="<?php echo $page;?>" />
    	<table cellpadding="0" cellspacing="0">
        	...
        	<tr>
        		<th width="10%">ID</th>
           		<th width="23%"><?php echo $_lang['service_name'];?></th>
           		<th width="7%"><?php echo $_lang['quotation_yuan'];?></th>
<th width="7%"><?php echo $_lang['shopkeeper'];?></th>
                <th width="10%"><?php echo $_lang['belong_to_industry'];?></th>
                <th width="10%"><?php echo $_lang['location'];?></th>
<th width="6%"><?php echo $_lang['goods_status'];?></th>
            ...
            <tr class="item">
                ...
                <td><a target="_blank" href="<?php echo $_K['siteurl'];?>/index.php?do=service&sid=<?php echo $v['service_id'];?>"><?php echo kekezu::cutstr($v[title],44) ?></a></td>
                <td><?php if($v['price']) { ?><?php echo $v['price'];?><?php } else { ?><?php echo $_lang['no_quotation'];?><?php } ?></td>
<td><?php echo $v['username'];?></td> 
                <td><?php if($indus_arr[$v['indus_id']]['indus_name']) { ?><?php echo $indus_arr[$v['indus_id']]['indus_name'];?><?php } else { ?><?php echo $_lang['no_has'];?><?php } ?></td>
                <td><?php echo $v['area_range'];?></td>
                <td><?php echo $status_arr[$v['service_status']];?></td>
                <td><a href="index.php?do=<?php echo $do?>&model_id=<?php echo $model_id?>&view=edit&service_id=<?php echo $v['service_id']?>&page=<?php echo $page;?>" class="button dbl_target"><span class="pen icon"></span><?php echo $_lang['edit'];?></a>
...
<a href="<?php echo $url_str;?>&ac=pass&service_id=<?php echo $v['service_id'];?>&page=<?php echo $page;?>"  onclick="return cdel(this,'<?php echo $_lang['confirm_pass_audit'];?>');" class="button"><span class="cog icon"></span><?php echo $_lang['audit'];?></a>
<?php } ?>
...
<?php } ?>
<?php if($v['service_status']==4) { ?>
<a href="<?php echo $url_str;?>&ac=use&service_id=<?php echo $v['service_id'];?>&page=<?php echo $page;?>"  onclick="return cdel(this,'<?php echo $_lang['confirm_use'];?>');" class="button"><span class="cog icon"></span><?php echo $_lang['use'];?></a>
<?php } ?>
</td>
            </tr>
            <?php } } ?> 
