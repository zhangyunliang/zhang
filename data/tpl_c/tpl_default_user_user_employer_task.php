<?php keke_tpl_class::checkrefresh('tpl/default/user/user_employer_task', '1465984392' );?><?php if($_K['inajax']) { ?>
                    ...
                    <div class="panel clearfix box">
                        ...
                        <div class="grid_21">
                            ...
                            <header class="clearfix box_header">
                                <h2 class="grid_4 alpha box_title t_c"><?php echo $_lang['task_manage'];?></h2>
                                <div class="grid_17 omega">
                                    <nav class="box_nav">
                                        <ul>
                                            <?php if(is_array($third_nav)) { foreach($third_nav as $v) { ?>
                                            <li <?php if($model_id==$v['1']) { ?>class="selectedLava"<?php } ?>>
                                                <a href="index.php?do=<?php echo $do;?>&view=<?php echo $view;?>&op=<?php echo $op;?>&model_id=<?php echo $v['1'];?>#userCenter" title="<?php echo $v['2'];?>"><?php echo $v['2'];?>(<?php echo $v['3']?>)</a>
                                            </li>
                                            <?php } } ?> 
                                        </ul>
                                    </nav>
...
<div class="grid_18">
                                    <?php echo $_lang['this_week_release'];?><?php echo $model_list[$model_id]['model_name'];?><?php echo $_lang['task_number'];?> :<?php echo $pub_count;?><?php echo $_lang['ge'];?><?php echo $_lang['zh_jh'];?>
                                	</div>
</div>
                                <!--messages消息 end-->
                                <div class="">
                                    ...
                                    <div class="clearfix box pad10">
                                        <form action="<?php echo $url;?>#userCenter" method="post" id="frm">
                                            ...
                                            <div class="rowElem clearfix">
                                                <div class="mr_5 fl_l">
                                                    <input type="text" name="task_id" class="togg_u txt_input" value="<?php echo $_lang['please_input_task_id'];?>" title="<?php echo $_lang['please_input_task_id'];?>" size="13"/>
                                                </div>
                                                <div class="mr_10 fl_l">
                                                    <input type="text" name="task_title" class="togg_u txt_input" value="<?php echo $_lang['please_input_task_name'];?>" title="<?php echo $_lang['please_input_task_name'];?>"/>
                                                </div>
                                                <div class="fl_l">
                                                    <label class="t_l fl_l">
                                                        <?php echo $_lang['task_status'];?><?php echo $_lang['zh_mh'];?>
                                                    </label>
                                                    <select class="fl_l" name="status">
                                                        <?php if(is_array($status_arr)) { foreach($status_arr as $k => $v) { ?><option value="<?php echo $k?>" <?php if($k==$status) { ?>selected="selected"<?php } ?>><?php echo $v;?>  </option>
                                                        <?php } } ?>
                                                    </select>
                                                </div>
                                                <div class="fl_l">
                                                ...
                                                <div class="fl_r">
                                                    <button class="block" type="submit" name="sbt_search" value="<?php echo $_lang['search'];?>">
                                        ...
                                        <div class="data_list mt_10">
                                            <table cellpadding="0" cellspacing="0" class="data_table">
                                                ...
                                                <tbody>
                                                    <?php if($task_info) { ?>
                                                    <?php if(is_array($task_info)) { foreach($task_info as $v) { ?>
                                                    <tr>
                                                        <td>
                                                            <?php echo $v['task_id']?>
                                                        </td>
                                                        <td>
                                                            <?php $time=time(); ?><?php if($v['top_time']>$time) { ?> <img src="resource/img/ico/zd.png" alt="<?php echo $_lang['task_top'];?>" title="<?php echo $_lang['task_top'];?>"><?php } ?> 
                                                            <?php if($v['urgent_time']>$time) { ?><img src="resource/img/ico/jj.png" alt="<?php echo $_lang['task_urgent'];?>" title="<?php echo $_lang['task_urgent'];?>"><?php } ?><a href="index.php?do=task&task_id=<?php echo $v['task_id'];?>" title="<?php echo $v['task_title']?>"><?php echo kekezu::cutstr($v['task_title'],20) ?></a>
                                                        </td>
                                                        <td>
                                                          <?php if($v['model_id']==4||$v['model_id']==5) { ?>
                                                          <?php echo $cove_arr[$v['task_cash_coverage']]['cove_desc']?>
                                                          <?php } else { ?>
                                                           <?php echo $v['task_cash']?><?php echo $_lang['yuan'];?>
                                                        <?php } ?>
                                                        </td>
                                                        <td>
                                                        ...
                                                        <td>
                                                            <?php echo $status_arr[$v['task_status']]?>
                                                        </td>
                                                        <td>
                                                            <a class="button" href="index.php?do=task&task_id=<?php echo $v['task_id'];?>"><span class="book icon"></span><?php echo $_lang['view'];?></a>
                                                            <?php if($v['task_status']=='0') { ?>
<a class="button" onclick="return del(this);" href="<?php echo $url;?>&ac=del&task_id=<?php echo $v['task_id']?>"><span class="trash icon"></span><?php echo $_lang['delete'];?></a>
                                                            <a class="button" href="index.php?do=<?php echo $do;?>&view=<?php echo $view;?>&task_id=<?php echo $v['task_id'];?>&model_id=<?php echo $model_id;?>&ac=pay"><span class="loop  icon"></span><?php echo $_lang['pay'];?></a>
                                                        <?php } ?>
                                                            <?php if($v['task_status']=='2') { ?>
<a href="index.php?do=task&task_id=<?php echo $v['task_id']?>&view=tools" class="button"><?php echo $_lang['use_tools'];?></a>
                                                            <?php } ?>
<a href="index.php?do=release&t_id=<?php echo $v['task_id']?>&model_id=<?php echo $v['model_id'];?>&pub_mode=onekey" class="button"><?php echo $_lang['one_key_pub'];?></a>
</td>
                                                    </tr>
                                                    <?php } } ?>
                                                    <?php } else { ?>
                                                ...
                                                <tfoot>
                                                    <tr>
                                                        <td colspan="6" class="t_r">
                                                            <?php echo $_lang['total'];?>:<?php echo $count;?><?php echo $_lang['ge'];?>
                                                        </td>
                                                    </tr>
                                                </tfoot>
                                            </table>
