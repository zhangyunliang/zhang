...
class keke_loaddata_class {
	...
	static function ad_show($code, $do = 'index',$is_slide=null) {
		global $_lang,$_K;
		$ad_target = db_factory::get_one ( sprintf ( " select * from %switkey_ad_target where code='%s' and INSTR(targets,'%s') and is_allow=1", TABLEPRE, $code, $do ),3600*24);
		if ($ad_target) {
			if($is_slide){
			    return self::adgroup($is_slide,$ad_target ['ad_num']);
			}
			$pos_config = unserialize ( $ad_target ['position'] ); 
