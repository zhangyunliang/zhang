...
abstract class keke_auth_base_class {
	...
	public function del_auth($auth_ids,$url=null) {
		...
		if($size==1&&$auth_info[0]['auth_status']!='1'){
			...
			$res = $this->del_auth_record($auth_info[0]['uid']);
		...
		$res && kekezu::admin_show_msg($this->auth_lang(). $_lang['apply_delete_success'],$url,3,'','success');
		kekezu::admin_show_msg($this->auth_lang().$_lang['delete_fail'],$url,3,'','warning');
	}
