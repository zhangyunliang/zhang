...
function pcwsetInvitation($data){
	
	
	if(session('?invitation_person')){
		
		session('invitation_person',$data['invitation_person']);
	}
}

function pcwsetCity($data){
	
	
	
	$area = D('ecs_GoodsArea');
	
	switch ($data['name']) {
		case '上海':
			$res = $area->get(array('goods_area'=>array('LIKE','%'.$data['name'].'%')));
			break;
		case '南京':
			$res = $area->get(array('goods_area'=>array('LIKE','%'.$data['name'].'%')));
			break;	
		default:
			$res = $area->get(array('goods_area'=>array('LIKE','%上海%')));
			break;
	}
	
	session('area_id',$res['goods_area_id']);
	
} 

function pcwsetAccess($data){
	
	session('temp_buyers_id',$data['temp_buyers_id']);
	session('temp_buyers_mobile',$data['temp_buyers_mobile']);
	session('suppliers_id',isset($data['ecs_RealSupplier']['suppliers_id'])?$data['ecs_RealSupplier']['suppliers_id']:-1);
	session('suppliers_mobile',isset($data['ecs_RealSupplier']['suppliers_mobile'])?$data['ecs_RealSupplier']['suppliers_mobile']:-1);
	
	pcwsetInvitation($data);
} 
 ...
 * @return 随机数
 */
function ecs_random($length = 6 , $numeric = 0) {
