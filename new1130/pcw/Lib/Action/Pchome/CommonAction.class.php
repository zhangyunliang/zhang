...
class CommonAction extends Action
{
	...
	protected function _initialize()
	{
		...
		if (!empty($this->uid)) {
			$this->member = d('ecs_TempBuyers')->get(array('temp_buyers_id'=>$this->uid));
			
			pcwsetAccess($this->member);//品材网 设置session
		}
		pcwsetCity($this->city);//品材网 设置
		...
		$this->ecs_goods_category = d('ecs_GoodsCategory')->fetchAll("*",array('pc_is_show'=>1));
		$this->assign('ecs_goods_category', $this->ecs_goods_category);
		$this->ecs_goods_category_slim = d('ecs_GoodsCategory')->fetchAll("*",array('goods_category_id'=>array('neq',106),'pc_is_show'=>1),array(0,3));
		$this->assign('ecs_goods_category_slim', $this->ecs_goods_category_slim);
	...
	protected function baoSuccessSubmit()
	{
		$parent = 'parent.';
		$str = '<script>';
		
	 	$str .= $parent.'document.getElementById("register1").removeAttribute("target");';
		
		$str .= $parent.'document.getElementById("register1_success").setAttribute("value","1");';
		
		$str .= $parent.'document.getElementById("register1").submit();';
